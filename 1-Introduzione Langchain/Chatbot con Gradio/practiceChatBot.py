...
# Prompt template
system_template = "Act like a useful assistant and answer the user questions using the information the user gives to you during the conversation."
...
def stream_response(session_id, input_text, history):
    ...
    # Mostra subito il messaggio dell'utente e svuota la textbox una volta sola,
    # senza aspettare il primo token dal modello (fix bug 1).
    # Il cursore ▌ funge da loading indicator finché non arriva il primo token.
    yield session_id, "", history + [
    ...
    try:
        ...
        stream = chain_with_history.stream(
            {"input": input_text}, config={"session_id": str(session_id)}
        )
        ...
        for token in stream:
            ...
            if delta:
                ...
                # gr.update() senza argomenti = "non toccare la textbox",
                # così l'utente può già scrivere il prossimo messaggio (fix bug 2)
                yield session_id, gr.update(), temp_history
...
with gr.Blocks() as demo:
    ...
    session_id_state = gr.State(
        value=lambda: f"{randrange(10000, 99999)}_{int(time.time() * 1000)}_{randrange(10000, 99999)}"
    )
...
demo.queue().launch(
    theme="ocean", debug=True, share=True, css="""footer{display:none !important}"""
)
